...
SDK_ProcAmp_Presets.h         # 自動生成されたプリセット定義（C++）
SDK_ProcAmp.h                 # プリセット利用側のメインヘッダー
```
...
#### 現在の問題点
色プリセットは現在、`SDK_ProcAmp.h` 内に**ハードコーディング**されています：

**場所**: `SDK_ProcAmp.h` 行542-708
...
SDK_ProcAmp_ColorPresets.h     # NEW: 自動生成される色プリセット定義
SDK_ProcAmp.h                  # MODIFY: 既存のハードコード削除、新ヘッダーをinclude
```
...
#### 変更箇所
- **削除**: 行537-748（約211行）
- **追加**: `#include "SDK_ProcAmp_ColorPresets.h"` 1行
- **影響**: 既存コードとの互換性100%維持
...
# 3. 自動的に SDK_ProcAmp_ColorPresets.h が更新される

# 4. SDK_ProcAmp.h の enum ColorPreset に新しい定数を追加
# enum ColorPreset {
#     ...
...
| SDK_ProcAmp.h修正（ハードコード削除、include追加） | ⭐ | 30分 |
| 動作確認・テスト | ⭐⭐ | 1-2時間 |
| ドキュメント作成 | ⭐ | 1時間 |
| **合計** | | **6-9時間** |
...
- `SDK_ProcAmp_Presets.h` - 自動生成ヘッダーの例

### 新規作成予定ファイル
- `color_presets.tsv` - 色プリセットデータ
- `color_preset_converter.py` - 色プリセット変換スクリプト
- `SDK_ProcAmp_ColorPresets.h` - 自動生成される色プリセットヘッダー

### 修正予定ファイル
- `SDK_ProcAmp.h` - ハードコード削除、include追加
...
現在の `SDK_ProcAmp.h` から色データを自動抽出するスクリプトも作成可能：

```python
# extract_color_presets.py
"""
Extract existing color presets from SDK_ProcAmp.h and generate TSV
"""

import re

def extract_presets_from_header(header_path):
    """Parse SDK_ProcAmp.h and extract color preset definitions"""
    # 正規表現でプリセット定義を抽出
