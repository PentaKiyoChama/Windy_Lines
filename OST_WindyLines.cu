#ifndef OST_WINDYLINES_CU
#define OST_WINDYLINES_CU

// DEBUG RENDER MARKERS for CUDA (must match OST_WindyLines.h)
				...
				if (coverage > 0.0f)
				{
					...
					pixel.w = prevAlpha + (outAlpha - prevAlpha) * appearAlpha;
						...
						else
						{
							...
							// Use un-premultiplied accumulation (same as OpenCL)
							float srcAlpha = coverage;
